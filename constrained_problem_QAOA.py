...
def MIS_QAOA(no_vertices, depth, G, use_constrain_operator, penalty_term = 1):
    ...
    if use_constrain_operator:
        ...
        def obj_func(parameter_values):
            dens_mat = unconstrained_operators.build_MIS_unconstrained_QAOAnsatz(G, parameter_values, pauli_ops_dict)
            expectation_value = (hamiltonian * dens_mat).trace().real
            return expectation_value * (-1.0)
        ...
        optimal_para = list(result.x)
        dens_mat = unconstrained_operators.build_MIS_unconstrained_QAOAnsatz(G, optimal_para, pauli_ops_dict)
    ...
    print('maximum hamiltonian eigenvalue:', max_ham_eigenvalue)
