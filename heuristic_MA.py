...
def random_select_MA(no_vertices, depth, seed, graph_type, save = True):
    """
...
def select_layer2_MA(no_vertices, seed, graph_type, save = True):
    ...
    select nodes to be 0 (from largest drgee 
    and its edges that connect all other -4/pi nodes
    layer1 connect 0 veterices
    layer2 connect other veterices
    '''
    gamma_0 = -0.7854
    beta_0 = 0.7854
    graph = generate_graphs.generate_graph_type(no_vertices, graph_type, seed)[0]
    ...
    dens_mat = build_operators.build_heuristic_MA_qaoa_ansatz(target_graph, parameter_list, selected_v, pauli_ops_dict, 'All')
    hamiltonian_expectation = (hamiltonian * dens_mat).trace().real
    cut_approx_ratio = (hamiltonian_expectation + max_cut_value - max_ham_eigenvalue) / max_cut_value
