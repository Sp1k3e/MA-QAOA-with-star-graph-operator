...
mix_hamiltonian_Y = build_operators.mix_hamiltonian_Y(graph)
# print(mix_hamiltonian_Y)
...
def build_layer(curr_dens_mat,beta, mix_type):
    ...
    mix_unit = build_operators.mixer_unitary(mix_type, beta * delta_t, pauli_ops_dict, no_vertices, 0)
    curr_dens_mat = (mix_unit * curr_dens_mat) * (mix_unit.transpose().conj())

    return curr_dens_mat


def update_beta(curr_dens_mat, mix_hamiltonian):
    A = 1j*(mix_hamiltonian*hamiltonian - hamiltonian * mix_hamiltonian) * curr_dens_mat
    expectation = abs(A.trace())
...
while layer < max_layers:
    layer +=1
    if layer % 2 == 1:
        curr_dens_mat = build_layer(curr_dens_mat, beta, "standard_x")
    else:
        curr_dens_mat = build_layer(curr_dens_mat, beta, "standard_y")
    ...
    if layer % 2 == 1:
        beta = update_beta(curr_dens_mat, mix_hamiltonian_Y)
    else:
        beta = update_beta(curr_dens_mat, mix_hamiltonian)
